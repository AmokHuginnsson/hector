...
miniHttpd=`echo ${SERVER_SOFTWARE} | grep "mini_httpd"`

if [ ! \( -z "${boa}" -a -z "${miniHttpd}" \) ] ; then
	filter="sed -e s@:SSL@ssl-redirect.cgi?REQUEST_URI=${REQUEST_URI}@"
else
	filter="cat"
fi
...
env -i \
	...
	./build/debug/hector.cgi/1exec "${@}" | ${filter}
