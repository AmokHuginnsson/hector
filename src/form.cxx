...
void HForm::commit( ORequest& req_, HSession& session_ ) {
	...
	if ( ok ) {
		...
		for ( HString const& col : writeColumns ) {
			...
			ORequest::value_t value( req_.lookup( input._htmlName, ORequest::ORIGIN::POST ) );
			if ( !! value ) {
				_crud[colNo] = ( input._type == OInput::TYPE::PASSWORD )
					? (
						_application.get_hash() == HApplication::HASH::SHA1
							? tools::hash::to_string( tools::hash::sha1( *value ) )
							: tools::hash::to_string( tools::hash::md5( *value ) )
					)
					: *value;
			} else {
